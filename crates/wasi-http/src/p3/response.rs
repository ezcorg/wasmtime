...
impl Response {
    ...
    pub fn from_http<B, E>(
        res: http::Response<B>,
    ) -> (Response, impl Future<Output = Result<(), ErrorCode>>)
    where
        ...
        E: Into<ErrorCode>,
    {
        ...
        let wasi_response = Response {
            ...
            // TODO: handle Body::Guest case when converting from http::Response to Response. 
            // The body is expected to be a Host body when constructing a Response from an http::Response.
            body: Body::Host {
                body: body.map_err(|e| e.into()).boxed_unsync(),
                result_tx,
            },
        };
